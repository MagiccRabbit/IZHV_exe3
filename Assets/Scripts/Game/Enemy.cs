...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if(player != null)
        {
            ...
            Quaternion look = Quaternion.LookRotation(transform.position, Vector3.forward);
            Vector3 direction = (player_pos - transform.position).normalized;
            mRigidBody.MovePosition(transform.position + direction/10.0f);
            mRigidBody.MoveRotation(look);
